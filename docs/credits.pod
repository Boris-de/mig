...
A huge thanks to Rasmus Lerdorf and all of the other fine folks who have
...
Yannick MassE<eacute>, for an updated French translation.
...
Jan Jasinski (jan at sps.ee) for contributing a patch to C<folderFrame()>
and for the Estonian translation.

Pawel Dylik (pawel at pavel.toya.net.pl) for contributing a patch which
resulted in the pagination of large galleries and the the random thumbnail
...
F<mkGallery.pl> do things in a sorted order.

Artem G. Abanov (aga at andrey.physics.wisc.edu), for writing the
script F<migPutCommentInFile> (available in the F<mig-contrib> package).

Jevin Maltais (jevin at techie.com) and Brian Palik (bpalik at elitepub.com),
for inspiring the C<#include> feature in the template parser.
...
Adam Monsen (meonkeys at hotmail.com), for reporting the E<quot>..E<quot>
security hole.
...
basis for the C<-c> option of F<mkGallery.pl>, and for the Portugese
translation.
