...
images in it (create thumbnails, if you want to), and you're done.
...
Plug-in mode for PHP-Nuke 4.x users.  (NOT tested with 5.x)

Scripts are included to create thumbnail images and extract embedded
...
actual images are (JPG, GIF, PNG, etc...).  By default, thumbnails are
the same type as the images they represent.
...
Spanish, Slovak, Dutch and Polish.
...
your own files in Mig templates (HTML, PHP, plain text, CGI, etc).
...
Pop-up windows can be enabled if so desired.

Maintainer's email address in each folder can be customized.
