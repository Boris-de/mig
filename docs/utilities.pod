...
=head2 Contrib Utilities

F<mkGallery.pl> - Create a gallery from images in the current directory.
This was a bundled utility that was moved to the a separate repository
that can be found at L<https://github.com/Boris-de/mig-contrib>.
...
For people who don't like to type in the same options to F<mkGallery.pl>
every time they use it, you may store memorized options in the file
F<mkGallery.opt> (this goes in the same directory as F<mkGallery.pl>).
Put the options in as you would type them at the command line.  For
example if you usually do this:

    ../../util/mkGallery -rantK
    
Then you would just put the '-rantK' in F<mkGallery.opt>, and
F<mkGallery.pl> will know to read it from that file.
