...
directory to build it and you will end up with an executable named
F<jhead>.

Since most Windows systems do not come with a C compiler, and my
understanding is that you have to buy one if you want one, I'd advise
Windows users to get the pre-built F<jhead> program from Mathias Wandel's
jhead home page: http://www.sentex.net/~mwandel/jhead/

See the E<quot>utilitiesE<quot> document for more information about
F<mkGallery> and F<jhead>.

If you are looking for information on the PHP-Nuke mode, see the F<phpnuke>
document.
...
Copy the file F<index.php> to your Mig directory.
...
F<templates> subdirectory to the F<templates> subdirectory you just made.
...
If you wish to create thumbnails, use the included F<mkGallery> script if
you have shell access to your server, and you have Perl and ImageMagick
available.  Otherwise you'll need to make your own thumbnails some other
way, or just not use thumbnails.

To install your own thumbnails, create a F<thumbs> subdirectory in each of
your album directories.  Put the thumbnail images into the F<thumbs>
subdirectory.  The filename of each thumbnail should be identical to the
filename of the full-size picture it represents.

You can change the name of the thumbnail directory - see the
C<$thumbSubdir> option in F<config.php>.
...
Copy the F<index.php> file to your Mig directory.  This should replace the
F<index.php> you already had there.
...
(mostly used as an example - you don't need config.php.default in your
actual Mig directory).

If you don't copy over F<config.php> and use Mig without it, Mig will use
...
was last updated, check the E<quot>changelogE<quot> document).

If you want F<mkGallery> to be able to use F<jhead>, put F<jhead> in the
same directory as F<mkGallery>.

=back

=head2 Running Mig Under Windows

Mig can run fine under Windows (so I'm told) as long as you have a web
server that supports PHP.  You can find the Windows version of PHP at
http://www.php.net/

Mig is reported to work with IIS, Personal Web Server, and the Windows port
of Apache.  One user noted that you needed a fairly recent version of PHP
...
album would cause problems, so you should avoid doing that.
...
project page at http://sf.net/projects/mig/ (look in the 'mig-extras'
section in the project files listing).  After you download the file, unpack
it in your albums directory and you should be able to view it in your web
browser.
...
You can edit the HTML templates found in the F<templates> subdirectory.
See the E<quot>templatesE<quot> document for more information.  You can
also use a custom per-folder template file in each folder if desired - see
the E<quot>mig_cfE<quot> document for more information.

=item 2. Comments

You can add a comment for any image you wish.  Comments are stored in a
file called F<mig.cf> in the same directory as the image.  See the
E<quot>mig_cfE<quot> document for information about the format of
F<mig.cf>.
...
but nothing in particular to say about the car.  You would add
this to F<mig.cf>:
...
(If you are using F<mkGallery>, you should look at the C<-c> option
which handles comments.)

You can have as many lines as you want for the description.  The key is
that the description must be enclosed inside the
E<lt>CommentE<gt>...E<lt>/CommentE<gt> structure.  The tags such as
E<lt>CommentE<gt> and E<lt>/CommentE<gt> MUST be at the start of a new
line.

=item 3. EXIF data

You can build EXIF data files (F<exif.inf>) using F<mkGallery> and the
C<-e> option.  See the E<quot>utilitiesE<quot> document.  Note that you
must have the F<jhead> utility in place before the C<-e> option will
function.  (See above during the install procedure.) Since only JPEG files
support EXIF headers, F<mkGallery> will only look at JPEG files when
building the EXIF files.
...
sorting).  Just define a E<lt>sortE<gt> block in the folder's F<mig.cf>
file and put items in the order you want them to be in.  For example:
...
What happens if there are files present other than the ones you listed?
They'll appear after those first three, sorted alphabetically.

You can sort folders, too, if your folder has subfolders.  Just define
another sort block with folders instead of images.  Mig will figure out
...
If you are interested in translating Mig to another language, please
contact me by email at I<dan at tangledhelix.com>.
