...
development platform is Apple OSX, so this code tends to assume a Unix-like
environment.  As such it may or may not work (and has not been tested) on
non-unix-like platforms.

(Windows users, please see the second labeled "FOR WINDOWS USERS" below.)

The F<mkGallery> utility (used to generate thumbnail images) requires Perl
( http://www.perl.com ) and, if you want it to general thumbnails, also needs
ImageMagick ( http://www.imagemagick.org/ ).  Perl and ImageMagick are not
needed to use Mig itself - only for use with mkGallery.  See
docs/Utilities.txt.
...
plan to extract EXIF data from your JPEG files.

=head2 INSTALLATION AND BASICS

Installation is fairly straightforward.

1. If you are installing Mig for the first time, you'll need to create a
directory for it to live in.  Otherwise, just know where your Mig
installation lives

4. Review the file F<config.php.default>.  If you want to make any changes,
make them and then copy the file to F<config.php>.  The reason to do this
is simple: future installations / upgrades will copy over
F<config.php.default> with whatever is packaged with that version of Mig.
The F<config.php> file will never be automatically over-written by the install
script.
...
Or, if you are upgrading, it's a good idea to review F<config.php.default> to
see if there is anything new that you will want to add to your F<config.php>.

(note to pre-1.3.1 users - the old "mig.cfg" file is now called
"config.php".  Rename your file - "mig.cfg" is now ignored.)
...
PHP files themselves, F<config.php.default>, the templates, etc).  If these
...
Thumbnails are the same filename as the images they represent, but they
live in a subdirectory called "thumbs" (this name can be changed by editing
config.php).
...
you want.  Just have them located in the "thumbs" subdirectory (or wherever
you pointed that directory with C<$thumbSubdir>).
...
C<$thumbExt> option in F<config.php>.  Also see F<docs/Utilities.txt>.
