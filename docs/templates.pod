...
In PHP-Nuke compatibility mode (see the E<quot>phpnukeE<quot> document)
then these three files are not used.  In their place will be:
...
Tags used only in F<folder.html> (or F<mig_folder.php> in PHP-Nuke mode)
...
Tags used only in F<image.html> (or F<mig_image.php> in PHP-Nuke mode)
...
(Because PHP-Nuke uses its own CSS, F<templates/style.css> is not used in
PHP-Nuke compatibility mode.)
