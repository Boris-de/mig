...
If you are using PHP-Nuke, PostNuke or phpWebThings, these three files will
not exist.  This is called E<quot>NukeE<quot> mode.  In their place you'll
find F<mig_folder.php> and F<mig_image.php> which are the same basically as
F<folder.html> and F<image.html> respectively.  In Nuke mode the CSS file
is not used (Nuke systems have their own CSS already).
...
Tags used only in F<folder.html> (or F<mig_folder.php> in Nuke mode)
...
Tags used only in F<image.html> (or F<mig_image.php> in Nuke mode)
