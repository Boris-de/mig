...
=head2 Version 1.3.2 (NOT YET RELEASED)
...
If F<config.php> is missing, Mig will move on and just use default values
instead.  Why anyone would want to do that is beyond me, but I think it's
simpler this way.
...
file (well, the PHP code is anyway), plus a config file written in PHP.
Perhaps not as nice and neat for people to look at, but people wanting nice
and neat are welcome to check out a CVS copy which is quite neat and tidy
and modular.
...
Deprecated C<$phpNukeRoot>.  Apparently at some point it disappeared from
the code and I forgot to take it out of config.php.
