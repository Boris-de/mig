...
=head2 MIG Change Log

=head2 Version 1.6.1 (tbd)

=head3 Important notes

The default encoding in C<$httpContentType> changed from "us-ascii" to "UTF-8". As ASCII is a subset of UTF-8 this
should not be a problem for a default setup, but together with the introduction of the C<$migCharset> config option
it can make a difference if you have used C<$httpContentType> to configure a charset that is neither ASCII nor UTF-8.
If that's the case you MUST set C<$migCharset> according to your C<$httpContentType> setting. If not, it is still
...
C<config.php>. Unless you overwrite your C<config.php> with every release nothing should change for you, but if you
do, please double-check the new options and set them according to your needs.

=head3 Notable changes

Mig now has support for PHP's open_basedir configuration. It's off by default, but enabled in the default C<config.php>.
If you want to enable this feature, check the new options in that file.

The encoding in the C<$httpContentType> setting of the default C<config.php> changed from "us-ascii" to "UTF-8". See
notes above.

There is a new option C<$migCharset> to specify the charset/encoding that should be used by Mig. The default for this
setting is "UTF-8", see notes above.

The contrib folder was moved to a separate repository and can now be found at L<https://github.com/Boris-de/mig-contrib>
...
=head2 Version 1.6.0 (Sun, 10 Oct 2020)

=head3 Important changes
...
Support for embedding into portals was removed. This feature hasn't been tested in over 15 years, some of the portals
seemed to discontinued. If you are using mig in one of the portals that still get updated, please let me know and we can
...
of jhead in the distribution of your choice or the latest sourcecode from L<https://www.sentex.ca/~mwandel/jhead/>
...
Code is now hosted on github (https://github.com/Boris-de/mig) to avoid having to setup a self-hosted git.
Github actions are used to run the test with several PHP versions as well.

=head2 Version 1.5.5 (Sat, 07 Oct 2017)
...
=head2 Version 1.5.4 (Tue, 29 Dec 2015)
...
The config option C<$suppressImageInfo> did not work anymore since 1.4.0. This is fixed now. Please note that this option is deprecated anyway and can easily be replaced with C<$fileInfoFormatString>. The documentation for that is now fixed too in config.php.

Reading image descriptions from files per image did not work anymore, apparently since 1.4.0 too. This is fixed now.

Use the same values in the defaults.php that are used in config.php for C<$imageFilenameRegexpr> and C<$currDirNameRegexpr>. This also fixes an error with PHP 5.6 installations which choke on the default regex because of their use of an unescaped hyphen in the middle of the expression, which was accepted in previous versions.
...
Changed file-type detection to use the new config options C<$image_extensions>, C<$video_extensions> and C<$audio_extensions>. You can use these options to add currently unhandled or new file formats.

Fixed some code-style issues


=head2 Version 1.5.3 (Thu, 16 May 2013)
...
A feature has been added to optionally restore the old behavior regarding 'total_images'. If you set C<$showTotalImagesString = false> in your config, the total_images text will not be generated, which is effectivly the same behavior that existed between 1.4.0 and 1.5.2.

=head2 Version 1.5.2 (Sat, 06 Aug 2011)
...
Releases and release-tags in the SCM will now be signed with GPG. You can find the current key at L<https://mig.wcht.de/downloads/KEYS>
The fingerprint of the current key is C<F5A4 0D2C 0DFF 4E5B 4908 198B 4709 198D 7464 C7A7> and it should be signed by cacert.

=head3 Incompatilities

If you use C<$pathConvertRegex> (which should be very few), you'll have to take care that your regex will work with preg_replace!
This should affect very few installations, that have to modify index.php on every update anyway.

=head2 Version 1.5.1 (Tue, 24 Nov 2009)
...
more liberal C<$imageFilenameRegexpr> and C<$currDirNameRegexpr>, which made problems with special foreign characters (Boris)
...
new PNG images (public domain) taken from L<http://tango.freedesktop.org/> Thanks to the people from the Tango Project.
The old images (or any other) however can still be enabled with the config variables C<$movie_icon>, C<$folder_icon>, C<$music_icon> and C<$nothumb_icon> (Boris)

=head3 Organisational

Switched Source Code Management from CVS to Mercurial. The history of the default branch was imported
The CVS is still available and I intent to integrate the release-patches for users that update their version from CVS.
But I<live>-changes will only happen in Mercurial. Mercurial is very easy with simple workflows, just give it a try. (Boris)

=head2 Version 1.5.0 (Mon, 15 Aug 2005)
...
introduce C<$imageFilenameRegexpr> and C<$currDirNameRegexpr> for a more secure handling of file- and directorynames (now PHP 3.0.9 is required!) (Boris)
...
never show an empty folder list, if C<startfrom> is bigger then the amount of pages (Boris)

If an non-image is viewed with pageType=image or pageType=large, a generic will be displayed and link to the file. (Boris)

added support for user-defined Content-Type with C<$httpContentType> (Boris)
...
=head2 Version 1.4.1 (Wed, 06 Apr 2005)

[Sourceforge bug report #974964] - Fixed bug in C<buildDirList()>
and C<buildImageList()> where an array pointer was not reset after
use. (Dan)
...
=head2 Version 1.4.0 (Mon, 01 Mar 2004)
...
Added F<config.php> options C<$prevFormatString> and C<$nextFormatString>.
Now individual pics or text is possible for the "previous image" and "next image" in the image-view (Werner)

Moved brackets [] from the prev-next-back to thumbnail-view - large image to F<templates/image.html>. If you don't need large images, please edit this file, otherwise some brackets are shown, but no link... (Werner)

Added F<config.php> options C<$fileInfoFormatString>.
C<$suppressImageInfo> is not needed any more and will be removed in Version 2.0  (Werner)

Added F<config.php> options C<$imagePopMaxWidth> and C<$imagePopMaxHeight>
...
by C<$largeSubdir> (by default, this is "large").  This entire feature is
off by default.  To turn it on, set C<$useLargeImages> to C<TRUE>.

Added new template tags to support large images: C<largeLink>,
C<largeHrefStart>, C<largeHrefEnd>, C<largeLinkBorder>.  See the
F<templates> document for more details.  These four tags must be present in
your F<image.html> or F<mig_image.php> template for large image support to
function.  Either copy the new F<image.html> over your old one or use it as
a guide to modify your custom template.

Added new template files F<large.html> and F<mig_large.php> to support
...
templates directory - F<large.html> for normal users, F<mig_large.php> for
users of PHP-Nuke, et al.

Added new F<config.php> options to support large images:
C<$useLargeImages>, C<$largeSubdir>, C<$largeLinkFromMedium>,
C<$largeLinkUseBorders>.  See F<config.php> for more details.

Added new language tags C<largeview> and C<largelink> to each translation
to support large images.

Modified F<mkGallery.pl> with regard to EXIF behavior.  Now if C<-n> and
C<-e> are both used, only files not already cached in F<exif.inf> will be
examined for EXIF data.  This has no effect if C<-w> is also used, because
in that case the F<exif.inf> file is removed before processing takes place,
and the cache is therefore empty.  Also changed the flags passed to
F<jhead> since the existing behavior using C<-v> (verbose) made no sense.
It just added extra junk to the cache file that served no useful purpose.
Improved the code to find the location of F<jhead> by adding its directory
to the execution path temporarily during F<mkGallery.pl> execution.

C<$pathConvertFlag>, C<$pathConvertRegex> and C<$pathConvertTarget>
have moved to F<index.php>.  This is because they are used to
include F<config.php>, so obviously they can't be located inside it.

Fixed bug in C<getImageDescFromFile()> that was not URL-decoding filenames.

Added Geeklog compatibility mode.  See F<config.php> and the F<portals>
document.

Reversed the order of entries in the F<credits> document - I always felt
...
the top and will be more visible (although will still sink down the list
...
F<music.gif> and F<movie.gif> (found in the F<images> folder).
...
Fixed bug in C<getRandomThumb()> that caused random thumbnail selection to
fail when not using thumbnail subdirectories.

Removed stray C<BR> element causing spacing problems in
C<getRandomThumb()>.

Removed requirement for there to be a F<config.php> file - now if there is no
F<config.php> the defaults will be used instead, and Mig will not error out
and exit.
...
the C<$mig_config> array, rather than passing as parameters to functions.
Most functions no longer have enormously long parameter lists.

Added a call to C<stripslashes()> to get rid of the extra backslashes
found in Windows SMB share paths.

Optimized parts of C<buildImageURL()> which were doing the same string
manipulations in either result of an if/else test.

Got rid of a misplaced backslash in C<buildImageURL()>.  This may fix a bug
...
Fixed bug in handling of E<lt>ShortE<gt> blocks.

Fixed bug in Exif format string handling.

Added feature to F<mkGallery.pl>: options can be memorized by putting them
in F<mkGallery.opt>.  See the utilities documentation.

=head2 Version 1.3.8 (Fri, 25 Oct 2002)

If Mig is called with no parameters, it will redirect to itself with
C<currDir=.> appended.  This makes it easier to tack on more parameters
later - no need to worry about the difference between & and ?.

Added C<newLang> to the template parser.  This makes it trivial to add
links to your templates which switch between languages on the fly.  See the
install documentation for more details.

Added C<$omitImageName> to F<config.php>.  When C<TRUE> it will cause the
image name to be omitted from the path line in image views.

[Sourceforge bug report #626032] -
Fixed the '..' check for C<currDir>.  At some point it had been moved down
...
PostNuke 0.7.2.1_Phoenix, PHPWebSite 0.8.3, PHPWebThings 0.5test7.

Renamed the F<phpnuke> document to F<portals>.

Added integration with Xoops content management system using patches
provided by Marius Scurtescu.  See the F<portals> document.

Files formerly found in F<templates/phpnuke> will now be in
F<templates/portals>.

Added Chinese big5 translation (zh).

=head2 Version 1.3.7 (Sat, 19 Oct 2002)
...
Added C<mig_dl> parameter to URL parser - see the install document for more
on this feature.  It lets you translate Mig into any language on the fly,
regardless of the setting of C<$mig_language>.

Added C<$folderSortType> to F<config.php>.  This option, if set, will
override the value of C<$sortType> when sorting folders.

Changed F<mkGallery.pl> to strip profiles from thumbnails by default.
...
calling F<mkGallery.pl> with the C<-K> flag.

=head2 Version 1.3.6 (Mon, 30 Sep 2002)

Updated Danish, Italian and German translations.

Added Polish ISO-8859-2 translation (pliso).

Added E<lt>acronymE<gt> tag to files/folders counts.

Changed numeric character entities (i.e. E<amp>#225;) to alphabetic
references (i.e. E<amp>aacute;).

Upgraded included jhead distribution to version 1.8.

Added C<$ignoreDotDirectories> option to F<config.php>.

[Sourceforge bug report #589652] -
Fixed URL encoding bug in getRandomThumb()

[Sourceforge bug report #591337] -
Fixed bug in handling of E<lt>SortE<gt> and E<lt>HiddenE<gt>.  Previously
if an object was in a E<lt>SortE<gt> block, its status as a hidden item
would be ignored.  Objects marked as hidden are now hidden regardless of
how they are sorted.

Removed leading and trailing E<lt>STYLEE<gt> tags from
F<templates/style.css> - since Mig is using E<lt>LINKE<gt> to refer to the
style sheet now it was no longer necessary (and the W3C validator didn't
like it).  Fixed other minor HTML/CSS bugs.

Modified C<printTemplate()> to recognize SMB shares in Windows when
determining whether or not to prepend full path.

[Sourceforge bug report #585312] -
Changed C<getRandomThumb()> to actually get a random thumb, instead of
just returning the first thumb found.  This is optional behavior - those
preferring the old way can set C<$useRealRandThumbs> to C<FALSE>.  NOTE:
this doesn't honor E<lt>HiddenE<gt> tags - things that are hidden may
still show up using C<$randomFolderThumbs>!  This is convoluted and I don't
plan to fix that bug, so don't use C<$randomFolderThumbs> if you want to
use E<lt>HiddenE<gt> too!

=head2 Version 1.3.5 (Sun, 07 Jul 2002)

Fixed bug in F<config.php> - the C<$pathConvert*> variables were
incorrectly named as C<$convertPath*> - Doh!

Fixed bug in C<FolderIcon> directive.  It had broken at some point and was
no longer properly pointing to the F<images> directory for icon files.

Fixed a bug where pre-sorted image lists would break pagination.

Added C<UseThumb> keyword to F<mig.cf> parser.
...
C<buildTable()> function.   Almost all E<lt>fontE<gt> tags have been
...
Applied updated French translation from Yannick MassE<eacute>.

Added option C<$commentFileShortComments> to F<config.php>.

Added E<lt>ShortE<gt> tags to F<mig.cf> parser.

Added C<$showShortOnThumbPage> to F<config.php>.

Fixed bug in C<getNumberOfDirs()> where it was not properly handling
thumbnail subdirectories.

=head2 Version 1.3.4 (Tue, 11 Jun 2002)

Added Estonian and Japanese translations.

Fixed formatting issue in C<folderFrame()> (fix provided by Jan Jasinski).

Changed all occurrences of C<NULL> to empty strings.  Apparently C<NULL>
was not introduced until PHP4, so using C<NULL> was breaking PHP3
compatibility.

Added number of folders in addition to number of images when
C<$viewFolderCount> is C<TRUE>.

Fixed a bug in C<getImageDescFromFile()> where a comment would be
incompletely read if the file did not end with a newline character.

Added F<config.php> options C<$convertPathFlag>, C<$convertPathRegex>,
and C<$convertPathTarget> for cases where weird C<include_path> munging
is required.  See near the bottom of F<config.php.default>.

Added new F<config.php> options C<$imagePopLocationBar>,
C<$imagePopMenuBar>, and C<$imagePopToolBar> to control various toolbars
being visible or hidden in pop-up windows.

Added C<is_dir()> checks before all calls to C<opendir()>.

Modified comment behavior when using C<$commentFilePerImage = TRUE> so that
if a comment file is not present, the comment from F<mig.cf> will be used
(if one is present).

=head2 Version 1.3.3 (Mon, 27 May 2002)

Apparently I didn't document it in the changelog, but at some point I added
code to C<printTemplate()> to check for drive letters in the pathname to
...
Updated F<phpnuke> document to reflect this.

Merged patch from Pawel Dylik to implement pagination of large galleries.
This resulted in the new F<config.php> option C<$maxThumbRows>, and a new
language element C<total_images> found in the language sets.  Right now,
...
Added C<$randomFolderThumbs> to F<config.php>.  If C<TRUE>, a random
thumbnail from the folder will be used as the icon rather than the generic
folder icon (unless a C<FolderIcon> is set in F<mig.cf> - that takes
...
Added C<$folderNameLength> to F<config.php>.  When C<$randomFolderThumbs>
is C<TRUE>, folder names are truncated if they are longer than
C<$folderNameLength> characters (to help with table formatting).
(Another Pawel patch.)

=head2 Version 1.3.2 (Tue, 21 May 2002)

Some major changes this release - please pay attention!

Eliminated F<install.sh> - all installs are now done the same way (Windows,
unix, FTP, shell prompt, whatever).  Eliminated F<FTPinstall> and
F<Windows> documents since they are now redundant.  New installation
procedure is documented in the F<install> document.

PHP-Nuke mode was no longer working, so I fixed it.  While I was at it,
I added PostNuke and phpWebThings compatibility.  See the F<phpnuke>
document.  Tested with PHP-Nuke 5.5, PostNuke 0.714 and phpWebThings 0.41.
Added config variables C<$phpWebThingsCompatible> and C<$phpWebThingsRoot>
to F<config.php> for the new phpWebThings mode.

Changed language designation for Danish to E<quot>dkE<quot> instead of
E<quot>daE<quot> (I don't know what E<quot>daE<quot> is, but it's not
Denmark).

Removed F<Example Gallery> from main distribution.  It will be available
...
Added C<MaintAddr> object to F<mig.cf> parser (thanks to Simon Wood for
supplying this code!).  This permits you to change the value of
C<$maintAddr> on a per-folder basis if desired.
...
Modified F<mkGallery.pl> to look for F<jhead> in E<quot>.E<quot> - simplifies
the task of finding it.

Entirely restructured the CVS tree.  When the smoke cleared, F<funcs.php>
and F<lang.php> had vanished (merged into F<index.php>).  Mig is just one
file now (well, the PHP code is anyway), plus a config file written in PHP.

Eliminated C<$useVirtual> - it was not being used by C<printTemplate()>
anymore.

Modified F<mkGallery.pl> to use thumbnail subdirectories by default.

Added C<-f> flag to F<mkGallery.pl>, so one can specify the path to the
F<config.php> file if so desired.

Deprecated the C<$viewCamInfo> and C<$viewDateInfo> options.  Everything
related to EXIF formatting is now controlled by the C<$exifFormatString>
variable found in F<config.php>.  Information on how to use it can be found
in F<config.php.default>.

=head2 Version 1.3.1 (Tue, 07 May 2002)

[Sourceforge bug report #467122] -
Fixed sorting bug (if C<$sortType> was set to anything other than
C<default>, the thumbnails would be sorted, but the images would
actually be out of order as far as the next/previous links were
concerned.  Turns out the sorting had made it into C<buildImageList()>
but not C<buildNextPrevLinks()>.

[Sourceforge bug report #512980] -
Removed two extraneous arguments in a call to C<printTemplate()>.
This hadn't caused me any trouble, but if C<error_reporting> was set
to include C<E_NOTICE>, errors would be generated.

[Sourceforge feature request #521109] -
Now when invoked with C<-c>, F<mkGallery> will also add a E<lt>BulletinE<gt>
tag to F<mig.cf>.  In interactive mode it will ask the user for the
bulletin text, otherwise it will just create an empty bulletin.

[Sourceforge feature request #521032] -
Added new config variable C<$commentFilePerImage> (default C<FALSE>).
When set to C<TRUE>, Mig looks for comments in a separate file for
each image rather than looking for all comments in F<mig.cf>.  For
example if you have F<IMG_4900.JPG>, Mig would look at F<IMG_4900.txt>
for the comment.  Unlike F<mig.cf>, a comment in one of these files
does not need to be enclosed in a E<lt>CommentE<gt> block.  The entire
file is used as the comment.  Mig will strip newlines and tabs out.
When C<$commentFilePerImage> is C<FALSE>, normal processing of comments
using the F<mig.cf> file happens.

Changed filename of configuration file from F<mig.cfg> to F<config.php>.
...
Changed default value of C<$useThumbSubdir> to TRUE.  Changed example
gallery to match.

Upgraded jhead to version 1.6.

=head2 Version 1.3.0 (Tue, 23 Apr 2002)

Added E<lt>TITLEE<gt> tags in thumbnail views so when hovering over a
thumbnail, you get the comment whether using Mozilla/Gecko/Netscape or
Internet Explorer.  C<$suppressAltTags> will also suppress the
E<lt>TITLEE<gt> tags.
...
Added date/time stamp parsing to C<getExifDescription()>.  This can be
activated by setting C<$viewDateInfo> to C<TRUE> in F<mig.cfg>.
(It is C<FALSE> by default).

Changed C<getExifDescription()> formatting slightly.
...
Fixed a bug in C<getExifDescription()>.  The string C<flash used>
...
=head2 Version 1.2.7p1 (Sun, 15 Apr 2001)

Fixed bug from 1.2.7 which caused images to show up as folders when both
folders and images were present.

=head2 Version 1.2.7 (Sat, 14 Apr 2001)

Added Danish translation.

Modified template parser so that you can use C<#include> to include PHP
files (files with extensions C<.php> or C<.php3>).

Added F<docs/FTPinstall.txt> for people who have only FTP access to their
server and no shell access (i.e. they can't run F<install.sh>).

Streamlined code where possible.

Got rid of repeated parses of F<mig.cf> in subroutines and created
C<parseMigCf()> to just do it once to save time and effort on file I/O.

=head2 Version 1.2.4 (Mon, 09 Apr 2001)

Added Swedish translation.

Removed all calls to C<die()> since they seemed to not work in all versions
of PHP.

=head2 Version 1.2.3 (Mon, 09 Apr 2001)

Removed F<install.bat>, rewrote F<docs/Windows.txt> to match.

Rewrote F<install.sh> for new directory layout (which was a side-effect of
making Windows installs easier).

Adjusted all server variables so that things should work whether
C<register_globals> or C<track_vars> are in use (or both).

Found a hard-coded directory in C<printTemplate()> - corrected.

=head2 Version 1.2.2 (Sun, 08 Apr 2001)
...
the C<$language> variable in F<mig.cfg.default>.
...
=head2 Version 1.1 (Thu, 28 Dec 2000)
...
=head2 Version 1.0 (Wed, 06 Dec 2000)

You can now type E<quot>quitE<quot> at any prompt during F<install.sh> to
abort.
...
=head2 Version 0.99 (Thu, 16 Nov 2000)
...
Added C<-n> and C<-r> switches to F<mkGallery>.  See F<docs/Utilities.txt>.
...
=head2 Version 0.98 (Sun, 05 Nov 2000)

Added jump URL tags.  See F<docs/Jump.txt> for more information.

=head2 Version 0.97 (Sat, 04 Nov 2000)
...
=head2 Version 0.96 (Thu, 02 Nov 2000)
...
=head2 Version 0.95 (Tue, 31 Oct 2000)
...
folks who had changed the value of C<$albumDir>.

Made the terms of the licensing clearer according to the FSF GNU
guidelines at L<https://www.gnu.org/copyleft/gpl.html>.
...
=head2 Version 0.94 (Sat, 28 Oct 2000)
...
Fixed bug in C<buildDirList()> which caused cascading links in the output
...
Changed link on image view pages that goes back to read E<quot>back to
thumbnail viewE<quot>.  Previously it read E<quot>back to folder
viewE<quot> but it has been pointed out to me that the wording was
confusing.
...
Updated all files to reflect new domain E<quot>tangledhelix.comE<quot>.
...
Added tag C<%%distURL%%> for templates.  Removed E<quot>MigE,<quot> from
E<lt>TITLEE<gt> in the default templates.
...
E<quot>_E<quot> (underscore) is now rendered as a space character in the
...
name to use (previously was forced to be E<quot>thE<quot>.  Also now you
can declare it to be a prefix (as in E<quot>th_filename.JPGE<quot>) or a
suffix (as in E<quot>filename_th.JPGE<quot>).  Default will be as previous,
suffix mode using E<quot>thE<quot> for filename_th.JPG.  See
F<docs/Utilities.txt> and F<docs/Install.txt> and F<mig.cfg.default> for
more information.
...
The pieces of the E<quot>you are hereE<quot> headline are now clickable for
...
Added a E<quot>now viewing #x of yE<quot> in image views, so the browser
...
Added E<quot>you are hereE<quot> line, but it's not clickable yet.

=head2 Version 0.60 (Sun, 06 Aug 2000)

Added E<quot>hiddenE<quot> feature.  Anything put into F<.hidden.txt> in a
...
Added E<quot>commentE<quot> feature.  Comments are stored in
F<.comments.txt>.  See F<INSTALL> for more information.

=head2 Version 0.51 (Sun, 06 Aug 2000)

Added C<$homeLink> and C<$homeLabel> to config, to print a E<quot>back to
{home}E<quot> link at the E<quot>rootE<quot> folder level.
...
Removed E<quot>backE<quot> link from E<quot>mainE<quot> or
E<quot>rootE<quot> of album.

=head2 Version 0.41 (Fri, 04 Aug 2000)

Fixed font size bug with back link on E<quot>mainE<quot> page.
...
Replaced E<quot>E<lt>E<lt>E<quot> in C<buildBackLink()> with C<&lt;&lt;>
...
Documented templates in F<templates/README.templates>.
...
present, the browser gets E<quot>No contentsE<quot>.  If no folders, the
...
The E<quot>backE<quot> link is now grayed out when at the
E<quot>mainE<quot> page.
...
Removed the E<quot>you are atE<quot> headline, the help function, and the
E<quot>exitE<quot> button.  The latter two were never too useful, and the
...
Fixed a bug which caused image view E<quot>backE<quot> links to go not to
the previous page, but literally to E<quot>../E<quot>.
...
Got E<quot>any directory anywhereE<quot> directory browsing working
...
Got the E<quot>you are hereE<quot> path to display, but no links yet.
...
Added the E<quot>exitE<quot> link to take browser to an outside URL.
