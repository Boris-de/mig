...
Doing a radical redesign of how Mig is installed.  Please review
documentation again before upgrading!

Changed language designation for Danish to 'dk' instead of 'da' (I don't
know what da is, but it's not Denmark).

Added C<MaintAddr> object to mig.cf parser (thanks to Simon Wood for
...
Removed Example Gallery from main distribution.  It will be available from
this point forward as a separate package on the site.  There's no reason
for every Mig download to be almost half a meg.

Did a general cleanup of documentation.
