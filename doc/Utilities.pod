...
(Note that F<jhead> is not for direct use - it is called by F<mkGallery>
when F<mkGallery> is invoked with the C<-e> flag).

F<convert-metafiles> and F<mf_conv.pl> are deprecated - they can be found
in the Mig-contrib package in the download section on the Mig site:
...
F<mkGallery> has NOT been tested under Microsoft Windows.
  ...
  Usage:
    mkGallery [ -h ] [ -a ] [ -w ] [ -t ] [ -e ] [ -c ] [ -i ]
      ...
      -D : Name of thumbnail subdirectory to use (default is "thumbs" or
           whatever is in your config.php file).  See docs/Utilities.txt.
...
=head2 Labels, prefixes, and suffixes

(As of 1.3.1, thumbnail subdirectories are the default - unless you've got
them turned off, they will take precedence).

The C<-M> and C<-m> options are new in 0.90.  In previous versions, all
thumbnails were built with the pattern F<filename_th.ext> so that an image
named F<myHouse.JPG> would have a thumbnail called F<myHouse_th.JPG>.

That's been changed.  You can now use any text you want for the label,
not just C<th>.  So you could have C<mkGallery -m mini> create thumbnails for
you like F<myHouse_mini.JPG> and F<myGarden_mini.JPG> instead.

Also, you can use a prefix rather than the default (a suffix).  So you
could use C<mkGallery -M prefix> to create thumbnails like
F<th_myHouse.JPG> and F<th_myGarden.JPG> instead.

If you choose to use prefixes and not suffixes, and/or a different label name,
you must make the associated changes to F<config.php>.  Otherwise Mig will not
know what to look for.  Changing F<config.php> has a side effect, too -
F<mkGallery> reads it to set defaults for C<-m> and C<-M>, so if your
F<config.php> is set to what you want, you shouldn't have to worry about C<-m>
or C<-M> when running F<mkGallery>.

Look at C<$markerType> and C<$markerLabel> in F<config.php.default> or
F<config.php>.  C<$markerType> equates to C<-M> and C<$markerLabel> equates to
C<-m>.

Values in F<config.php> are used, but if you invoke command-line options to
F<mkGallery>, C<-m> and C<-M> will override the values in F<config.php>.
...
See both the C<$thumbExt> option in F<config.php>, and the C<-E> option to
F<mkGallery>.  If you define C<$thumbExt> in F<config.php>, F<mkGallery> will
...
=head2 Using thumbnail subdirectories
...
instead.  To do this, set the C<$useThumbSubdir> and C<$thumbSubdir>
options in the F<config.php> file.  You can also use the C<-d> and C<-D>
options to F<mkGallery> to override what F<config.php> has configured.
...
the same filename for both, just in different directories.  Many people
view this as cleaner (I've converted my own gallery over to this method).
...
just type C<make> in the F<util/jhead> subdirectory.  If you don't have gcc
