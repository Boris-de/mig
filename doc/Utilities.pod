...
you must make the associated changes to F<mig.cfg>.  Otherwise MiG will not
know what to look for.  Changing F<mig.cfg> has a side effect, too -
F<mkGallery> reads it to set defaults for C<-m> and C<-M>, so if your
F<mig.cfg> is set to what you want, you shouldn't have to worry about C<-m>
or C<-M> when running F<mkGallery>.

Look at C<$markerType> and C<$markerLabel> in F<mig.cfg.default> or
F<mig.cfg>.  C<$markerType> equates to C<-M> and C<$markerLabel> equates to
C<-m>.

Values in F<mig.cfg> are used, but if you invoke command-line options to
F<mkGallery>, C<-m> and C<-M> will override the values in F<mig.cfg>.
...
See both the C<$thumbExt> option in F<mig.cfg>, and the C<-E> option to
F<mkGallery>.  If you define C<$thumbExt> in F<mig.cfg>, F<mkGallery> will
...
options in the F<mig.cfg> file.  You can also use the C<-d> and C<-D>
options to F<mkGallery> to override what F<mig.cfg> has configured.
