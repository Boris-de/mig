...
in the Mig-contrib package in the download section on the Mig site at
...
If you wish to generate thumbnails, the "convert" utility from the
ImageMagick suite is also required.)
  ...
  Usage:
    mkGallery [ -h ] [ -a ] [ -w ] [ -t ] [ -e ] [ -c ] [ -i ]
      ...
      -e : Build "exif.inf" file.
           See the file docs/Utilities.txt - you must compile the jhead
...
Thumbnails are stored in a subdirectory called "thumbs" by default.
(This can be changed - see the C<$thumbSubdir> option in F<config.php>.)
...
to its working with Linux, FreeBSD, Solaris and Apple OSX.
